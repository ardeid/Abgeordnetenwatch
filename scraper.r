...
#####################################################
#### Funktion zum Abgeordnetenwatch html Webscrapen. ####
#####################################################
...
#öffnet den Open Data Tab und liest daraus die Parlbez aus
if (length(parlbez) == 0) {
...
for(j in 1:length(paths)){
  ...
  for(k in 1:length(profile_links)){ 
    ...
    if (str_detect(parlbez, pattern = " Wahl ")) {
      frak <- html_elements(html, xpath = str_c("//div/h3[@class='accordion__title']/div[@class='accordion__title__text' and contains(text(),'", 
                                                parlbez,
                                                "')]/../..//div[@class='tooltip__text' and contains(text(), 'Angetreten für')]/a")) %>%
        html_text()
    }else{
      #Für Amtsperioden
      frak <- html_elements(html, xpath = str_c("//div/h3[@class='accordion__title']/div[@class='accordion__title__text' and contains(text(),'", 
                                                parlbez,
                                                "')]/../..//div[@class='tooltip__text' and contains(text(), 'Fraktion')]/a")) %>%
        html_text()
    }
    
    # Failsave für Parteilose 
    if (length(frak) < 1) {
      frak <- "Parteilos"
    }
    ...
    fraklink <- ifelse(length(fraklink) > 0, fraklink, NA)
    
    #Wahlliste oder Wahlkreis
    gewonnenüber <- html_elements(html, xpath = str_c("//div/h3[@class='accordion__title']/div[@class='accordion__title__text' and contains(text(),'", 
                                                      parlbez,
                                                      "')]/../..//div[@class='visually-hidden js-tooltip-text']",
                                                      #xpath für die spezifische Information im Info Tooltip
                                                      "//div[@class='field field--inline field--list_string field--mandate-won']")) %>% 
      html_text2() %>% 
    ...
    #Wahlkreis
    wkreis <- html_elements(html, xpath = str_c("//div/h3[@class='accordion__title']/div[@class='accordion__title__text' and contains(text(),'", 
                                                parlbez,
                                                "')]/../..//div[@class='visually-hidden js-tooltip-text']",
                                                #xpath für die spezifische Information im Info Tooltip
                                                "//div[@class='field field--inline field--entity_reference field--constituency']")) %>% 
      html_text2() %>% 
      #Info ohne Text davor
      str_extract(pattern = "(?<=\\n).+")
    ...
    # Wahlkreis Prozent  
    wkreisP <- html_elements(html, xpath = str_c("//div/h3[@class='accordion__title']/div[@class='accordion__title__text' and contains(text(),'", 
                                                 parlbez,
                                                 "')]/../..//div[@class='visually-hidden js-tooltip-text']",
                                                 #xpath für die spezifische Information im Info Tooltip
    ...
    #Wahlliste
    wliste <- html_elements(html, xpath = str_c("//div/h3[@class='accordion__title']/div[@class='accordion__title__text' and contains(text(),'", 
                                                parlbez,
                                                "')]/../..//div[@class='visually-hidden js-tooltip-text']",
                                                #xpath für die spezifische Information im Info Tooltip
                                                "//div[@class='field field--inline field--entity_reference field--ctoral-list']")) %>% 
      html_text2() %>% 
      #Info ohne Text davor
      str_extract(pattern = "(?<=\\n).+")
    #Umgang mit fehlenden Werten
    ...
    #Listenposition
    listpos <- html_elements(html, xpath = str_c("//div/h3[@class='accordion__title']/div[@class='accordion__title__text' and contains(text(),'", 
                                                 parlbez,
                                                 "')]/../..//div[@class='visually-hidden js-tooltip-text']",
                                                 #xpath für die spezifische Information im Info Tooltip
                                                 "//div[@class='field field--inline field--integer field--st-position']")) %>% 
      html_text2() %>% 
      #Info ohne Text davor
      str_extract(pattern = "(?<=\\n).+")
    ...
    qittr_years <- html_elements(html, xpath = str_c("//div/h3[@class='accordion__title']/div[@class='accordion__title__text' and contains(text(),'", 
                                                     parlbez,
