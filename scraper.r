...
aw_scraper <- function(parlamente, fragenscrapen = TRUE, maxretry = 5, save = FALSE, return = TRUE) {
  ...
  if (length(parlbez) == 0) {
    ...
    parlbez <- html_elements(html_parlbez, xpath = "//div[@class='page-title l-container']/h1") %>%
      html_text() %>%
      str_extract("^.+(?= \\()")
  }
  ...
  for (j in 1:length(paths)) {
    ...
    parlID <- html_elements(html, xpath = "//a[@class='use-ajax button button--secondary']") %>%
      html_attr("href") %>%
      str_extract("\\d+")
    ...
    for (k in 1:length(profile_links)) {
      ...
      if (str_detect(parlbez, pattern = " Wahl ")) {
        frak <- html_elements(html, xpath = str_c(
      ...
      gewonnenüber <- html_elements(html, xpath = str_c(
        "//div/h3[@class='accordion__title']/div[@class='accordion__title__text' and contains(text(),'",
        parlbez,
      ...
      # Wahlkreis
      wkreis <- html_elements(html, xpath = str_c(
        "//div/h3[@class='accordion__title']/div[@class='accordion__title__text' and contains(text(),'",
        parlbez,
        "')]/../..//div[@class='visually-hidden js-tooltip-text']",
        # xpath für die spezifische Information im Info Tooltip
      ...
      # Wahlliste
      wliste <- html_elements(html, xpath = str_c(
        "//div/h3[@class='accordion__title']/div[@class='accordion__title__text' and contains(text(),'",
        parlbez,
        "')]/../..//div[@class='visually-hidden js-tooltip-text']",
        # xpath für die spezifische Information im Info Tooltip
      ...
      # Listenposition
      listpos <- html_elements(html, xpath = str_c(
        "//div/h3[@class='accordion__title']/div[@class='accordion__title__text' and contains(text(),'",
        parlbez,
        "')]/../..//div[@class='visually-hidden js-tooltip-text']",
        # xpath für die spezifische Information im Info Tooltip
      ...
      qittr_years <- html_elements(html, xpath = str_c(
        "//div/h3[@class='accordion__title']/div[@class='accordion__title__text' and contains(text(),'",
        parlbez,
      ...
      flink <- html_elements(html, xpath = "//div[@class='tabs-mobile-select']//a/span[text()='Fragen und Antworten']/..") %>%
        html_attr("href") %>%
        str_c("https://www.abgeordnetenwatch.de", ., "?parliament_period=", parlID) %>%
        unique()
          ...
          tryCatch(
            {
              ...
              if (length(atext) > 1) {
                atext <- str_flatten(atext, collapse = " %>% ")
              }
              ...
              if (length(adate) > 1) {
                adate <- str_flatten(adate, collapse = " %>% ")
              }
