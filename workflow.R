...
if (any(str_detect(files, "parl_"))) {
  done <- files[str_detect(files, ".rds") & str_detect(files, "parl_")] %>%
    str_remove("parl_") %>%
    str_remove(".rds")
  ...
  if (i == length(files)) {
    ...
    if (any(results$quest_error)) {
      cat("Question Errors in Datasets:",
        results$parlbez[results$quest_error == TRUE],
        sep = "\n"
      )
    } else {
      cat("Question Errors in Datasets: \n-")
    ...
    if (any(results$prof_error)) {
      cat("\n", "Profile Errors in Datasets:",
        results$parlbez[results$prof_error == TRUE],
        sep = "\n"
      )
    } else {
      cat("Profile Errors in Datasets: \n-")
    }
  }
  # Memory Management
  rm(i, df)
